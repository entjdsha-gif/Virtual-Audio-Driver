...
- [x] V1 planning files in place (Step 2): PRD, ADR (12 at exit;
      ADR-013 added during Phase 0 review fix bundle; ADR-014 added
      between Phase 0 exit and Phase 1 entry to switch from
      single-branch to per-phase-branch model — supersedes ADR-012),
      ARCHITECTURE, DESIGN, REVIEW_POLICY, GIT_POLICY, CLAUDE.md,
      AGENTS.md, slash commands, phase scaffolding, execute.py.
...
- Clean planning structure: PRD locks goal, ADR-001..014 lock decisions
  (ADR-014 supersedes ADR-012 git model — Phase 1+ uses per-phase
  branches), ARCHITECTURE locks system shape, DESIGN locks file/struct/
  function shape.
