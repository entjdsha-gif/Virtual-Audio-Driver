# Phase 1 Step 1: Ring write — INT32 conversion (no SRC yet)
...
and added in Phase 2 — Step 1 only handles `srcRate == pipe->InternalRate`.
...
- `Source/Utilities/loopback.h` (signature stable from Step 0).
                       ...
                       ULONG srcRate, ULONG srcChannels, ULONG srcBits)
{
    ...
    if (srcRate != pipe->InternalRate /* same-rate only in Step 1 */) {
...
```powershell
python scripts/execute.py mark 1-int32-ring 1 completed --message "Same-rate INT32 ring write + 4-way bit-depth dispatch."
```
